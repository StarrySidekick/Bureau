...
## 143 · The specimen book is generated by the app, not written beside it

*2026-09-07*

Every visual option there is, on one page: the seven aesthetics, the sixteen
colour slots each of them names, the six slot families, every type drawn as the
thing it makes, every face a container can wear, every shape an object can be —
and the chrome, which nothing had ever laid out. `web/js/guide.js`, reachable
from a door in Settings.

`scripts/catalogue.mjs` was most of this already and had the right instinct
written at the top of it: nothing draws a tile itself, because a second renderer
drifts from the first the day a slot gains a rule. It then broke that rule at a
larger scale — the *page* was built out in the script, from data it extracted,
so the file on disk and anything the app might one day show were two books that
would agree until somebody edited one. The generator moved into the app and the
script became eleven lines that load a page and ask it for the string.

So there is one book with two destinations, and the app's copy is not a build
artifact at all: it is generated out of the desk that is running, at the moment
you press the button, which is the only version of "kept up to date" that
cannot be forgotten.

**It is shown in an iframe**, and that is a design decision rather than a
convenience. The book carries a global reset — `body` scrolls, is not
parchment, is not a hundred per cent tall — and Bureau's own stylesheet says all
three of those the other way round, because Bureau is an app and this is a page
you read. Inlined into the desk, one of them has to give way. Isolated, neither
does, and the same string is a file you can save.

**Its own host beside `#app`**, the way a panel lives on `#frame`, so
`render()` leaves it alone. Escape closes it before anything else, because
while it is up it is the whole screen.

Two things had to be worked around, and one of them is decision 98 from the
other side. The tile rules moved onto `<fam>sty-` classes, which is exactly what
lets seven aesthetics share a page; the **chrome** rules are still keyed on
`html[data-style]` and cannot be, because one page has one `<html>`. So every
one of those rules is re-emitted against a `[data-sty]` wrapper — read out of
the live CSSOM and rewritten, recursively, so a rule inside an `@media` keeps
its condition. Wholesale, not hand-picked: a rule about something the book does
not draw simply matches nothing, and choosing which ones mattered is how you
miss one. The failure is silent — a panel renders, just undressed — which is the
same failure `slotScoping` exists to catch, so `specimenBook` in the smoke test
compares Golf 97's panel against Victoria's and fails if they come out the same.

And the CSS is read from `document.styleSheets` rather than fetched. It needs no
network, so the book works offline; and what `cssRules` hands back is what this
browser *parsed*, which is a truer answer than the file on disk — a rule it
dropped was never applying to the desk either.

**The chrome specimens are written in the app's own class names, and that is
honest rather than a compromise.** A `.sqbtn` is not produced by a shared
function; it is markup written inline at a dozen call sites, so there is nothing
to reuse. This section *is* the inventory of those names, and a class that gets
renamed shows up here as an undressed specimen, which is exactly the thing a
style guide is for. Everything else on the page — every tile, every colour,
every token, every default — comes out of `sampleTile()` and off the root after
asking the app to *be* that aesthetic, so it cannot drift.

It earned its keep on its first run. The faces gallery drew nine identical
spirals: a container carrying `spawn` with no `spawnBy` of its own answers
`click`, and the spawner branch sits above every face. That was the sample's
