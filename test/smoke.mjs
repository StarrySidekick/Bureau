...
(async () => {
  ...
  const depth = await phone.evaluate(async () => {
    ...
    const ownBook = BUREAU.create('book', {title:'Depth', parent:'root'});
    if(ownBook){
      ownBook.phone = hereBox({x:Math.max(1, (shelfW>>1)-2), y:2, w:1, h:4});
      corners.push(ownBook);
    }
    out.thereIsABookToo = !!ownBook;
